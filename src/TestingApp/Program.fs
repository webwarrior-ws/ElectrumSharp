...
async {
    let! stratumClient = Electrum.CreateClient serverAddress serverPort timeout "geewallet"
    ...
    let! transaction = Electrum.GetBlockchainTransaction txHash (async { return stratumClient })
    printfn "transaction:\n%s" transaction
